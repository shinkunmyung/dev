<%--
  Class Name : qustnrItemManageList.jsp
...
function linkPage(pageNo){
	...
	document.listForm.action = "<c:url value='/survey/qim/qustnrItemManageList.do'/>";
...
function fn_egov_modify_QustnrItemManage(){
	location.href = "<c:url value='/survey/qim/qustnrItemManageModify.do' />";
}
/* ********************************************************
 * 상세회면 처리 함수
 ******************************************************** */
...
<form name="listForm" action="<c:url value='/survey/qim/qustnrItemManageList.do'/>" method="post" onSubmit="fn_egov_search_QustnrQestnManage(); return false;">
...
<%-- 				<span class="btn_b"> <a href="<c:url value='/survey/qim/qustnrItemManageRegist.do'/>" title="<spring:message code='button.create' /> <spring:message code='input.button' />"><spring:message code="button.create" /></a></span>  --%>
	...
	<c:forEach items="${resultList}" var="resultInfo" varStatus="status">
	  <tr>
		...
		<!-- 항목내용  -->
		<td class="lt_text3L">
		<a href="<c:url value='/survey/qim/qustnrItemManageDetail.do'/>?qustnrIemId=${resultInfo.qustnrIemId}" onClick="fn_egov_detail_QustnrItemManage('<c:out value="${resultInfo.qustnrIemId}"/>');return false;"><c:out value='${resultInfo.iemCn}'/></a>
		</td>
		<!-- 기타답변여부 -->
